...
export function DoubleSlit({ className }: { className?: string }) {
  ...
  const drawScene = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number) => {
    ...
    ctx.fillRect(layout.barrierX, layout.slitTop + layout.slitWidth, 6, layout.slitGap - layout.slitWidth * 2);
    ctx.fillRect(layout.barrierX, layout.slitBottom + layout.slitWidth, 6, h - layout.slitBottom - layout.slitWidth);
    ...
    if (mode === 'observe') {
      ...
      ctx.fillRect(layout.barrierX - 15, layout.slitTop - layout.slitWidth - 5, 12, layout.slitWidth * 2 + 10);
      ctx.fillRect(layout.barrierX - 15, layout.slitBottom - layout.slitWidth - 5, 12, layout.slitWidth * 2 + 10);
  ...
  const drawHits = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number) => {
    const layout = getLayout(w, h);
  ...
  return (
    <div className={cn('flex flex-col gap-4', className)}>
      ...
      <div className="rounded-lg border border-accent-cool/20 bg-accent-cool/5 p-3">
        <p className="text-xs text-fg-secondary">
          <span className="font-medium text-accent-cool">量子之谜：</span>
