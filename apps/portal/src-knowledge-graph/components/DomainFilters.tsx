    ...
    <div className="flex items-center gap-1.5">
      {showCloseButton && onClose && (
        <button
          type="button"
          onClick={onClose}
          className="flex h-7 w-7 items-center justify-center rounded-lg text-white/40 hover:text-white/70 transition-colors"
          aria-label="收起筛选栏"
        >
      ...
      {DOMAINS.map((domain) => {
        ...
        return (
          ...
          >
            <span
              className="h-2 w-2 rounded-full shrink-0 transition-opacity duration-200"
              style={{
