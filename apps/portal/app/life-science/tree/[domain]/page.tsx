...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description,
    openGraph: { title: `${domain.name}（${domain.nameEn}）— 生命之树`, description, images: [{ url: ogImage, width: 1200, height: 630 }] },
  };
}
      ...
      <div className="flex flex-col gap-12 lg:flex-row">
        <article className="min-w-0 flex-1 max-w-[1200px]">
          <header className="mb-12">
            ...
            <div className="flex flex-wrap items-center gap-2.5 mb-4">
              <span
                ...
                style={{ borderColor: `${domain.accent}30`, color: domain.accent, backgroundColor: `${domain.accent}10` }}
              >
                三域系统
              </span>
            </div>
          ...
          <FadeInSection className="mb-12">
            <h2 className="font-display text-fg-primary text-xl font-semibold mb-4" id="characteristics">
              核心特征
            </h2>
          ...
          <FadeInSection className="mb-12">
            <h2 className="font-display text-fg-primary text-xl font-semibold mb-6" id="key-phyla">
              主要门类
            </h2>
              ...
              {domain.keyPhyla.map((phylum) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-display text-fg-primary text-base font-semibold">{phylum.name}</h3>
                    <span className="text-fg-disabled font-mono text-[9px] italic">{phylum.nameEn}</span>
                  </div>
                  <p className="text-fg-secondary text-sm leading-relaxed mb-3">{phylum.description}</p>
                      ...
                      {phylum.representativeSpeciesIds.map((sid) => {
                        ...
                        return (
                          <Link
                            ...
                            className="rounded-full border px-2.5 py-0.5 font-mono text-[9px] tracking-[0.12em] transition-colors hover:border-accent-green hover:text-accent-green"
                            style={{ borderColor: `${domain.accent}25`, color: `${domain.accent}cc` }}
                          >
                            {sp.name}
                          </Link>
                        );
          ...
          {domain.representativeSpeciesIds.length > 0 && (
            <FadeInSection className="mb-12">
              <h2 className="font-display text-fg-primary text-xl font-semibold mb-6" id="representative-species">
                代表性物种
              </h2>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {domain.representativeSpeciesIds.map((sid) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-display text-fg-primary text-base font-semibold transition-colors duration-300 group-hover:text-accent-green">
                          {sp.name}
                        </h3>
                        <span className="text-fg-disabled font-mono text-[9px] italic">{sp.nameEn}</span>
                      </div>
                      <p className="text-fg-muted font-mono text-[10px] mb-2">{sp.era} · {sp.period}</p>
                      <div className="flex flex-wrap gap-1.5">
                        {sp.keyTraits.slice(0, 3).map((trait) => (
                          <span
                            key={trait}
                            className="rounded-full border px-2 py-0.5 font-mono text-[9px] tracking-[0.08em]"
                            style={{ borderColor: `${domain.accent}20`, color: `${domain.accent}aa` }}
                          >
                            {trait}
                          </span>
                        ))}
          ...
          <FadeInSection className="mb-12">
            <h2 className="font-display text-fg-primary text-xl font-semibold mb-4" id="evolutionary-significance">
              演化意义
            </h2>
            <div
            ...
            >
              <p className="text-fg-secondary text-sm leading-relaxed">{domain.evolutionarySignificance}</p>
            </div>
          </FadeInSection>

          <DeepReading {...domain.deepReading} />

          <div className="border-border-faint border-t pt-8 flex items-center justify-between gap-4 mt-16">
            <Link
            ...
            >
              ...
              <span className="text-fg-secondary group-hover:text-accent-green transition-colors">返回生命之树</span>
            </Link>
          </div>
        </article>
        ...
        <aside className="w-full lg:w-80 flex-shrink-0">
          <div className="sticky top-24 space-y-6">
            <nav className="border-border-faint bg-bg-near border p-5">
              <h3 className="font-display text-fg-primary text-sm font-semibold mb-4 tracking-wide">页面导航</h3>
              <ul className="space-y-2" role="list">
                <li>
                  <a href="#characteristics" className="text-fg-secondary hover:text-accent-green text-sm transition-colors">
                    核心特征
                  </a>
                </li>
                <li>
                  <a href="#key-phyla" className="text-fg-secondary hover:text-accent-green text-sm transition-colors">
                    主要门类
                  </a>
                </li>
                {domain.representativeSpeciesIds.length > 0 && (
                  <li>
                    <a href="#representative-species" className="text-fg-secondary hover:text-accent-green text-sm transition-colors">
                      代表性物种
                    </a>
                  </li>
                )}
                <li>
                  <a href="#evolutionary-significance" className="text-fg-secondary hover:text-accent-green text-sm transition-colors">
                    演化意义
                  </a>
                </li>
                <li>
                  <a href="#deep-reading" className="text-fg-secondary hover:text-accent-green text-sm transition-colors">
                    深度阅读
                  </a>
                </li>
              </ul>
            ...
            <div className="border-border-faint bg-bg-near border p-5">
              <h3 className="font-display text-fg-primary text-sm font-semibold mb-4 tracking-wide">三域系统</h3>
                  ...
                  <li key={d.id}>
                    ...
                    >
                      ...
                      <span>
                        ...
                        <span className="text-fg-muted ml-1.5 font-mono text-[10px] italic">{d.nameEn}</span>
                      </span>
                    </Link>
                  </li>
                ))}
