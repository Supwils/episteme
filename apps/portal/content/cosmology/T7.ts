  ...
  narrative: [
    ...
    },
    {
      ...
      body: [
        ...
        "载人航天的下一个里程碑是重返月球。Artemis II (预计 2025 年底) 将是自 Apollo 17 (1972) 以来首次载人绕月飞行；Artemis III 将在月球南极着陆，首次让女性和有色人种踏上月球。中国的嫦娥六号 (2024) 已经从月球背面带回了第一批样品，嫦娥七号 (2026) 将在南极寻找水冰。更远的目标是火星：SpaceX 的 Starship 被设计为火星运输工具，但载人火星任务至少需要 ~6 个月的单程飞行、~500 天的火星停留和辐射防护、生命维持等技术突破。Carl Sagan 说「无论你多么爱地球，总有一天你必须离开它」——不是因为地球不好，而是因为一个只住在一颗行星上的物种，在宇宙的时间尺度上是脆弱的。",
      ],
    },
    {
      heading: "「淡蓝色圆点」",
    ...
    },
    {
      ...
      body: [
        "2024 年 Pham et al. (Nature) 在地震学界引发了一次重大震动：通过分析全球 200 多个地震台站对重复地震 (doublets) 的 P 波到时差异，发现地球内核内部还存在一个「内内核 (inner inner core)」——半径约 650 km，其晶体各向异性的优先取向轴与外层内核的各向异性轴相差约 54°。这意味着内核不是一块均质的铁球，而是至少由两层不同晶体取向的固态铁组成。最可能的解释是：内内核形成于地球历史的更早期 (可能是 ~5 亿年前的一次地磁发电机重组事件)，其晶体在当时的热流条件下沿不同方向生长；外层内核在后续数十亿年中逐渐凝固，受到不同的应力场约束而形成了不同的取向。这一发现将内核的形成史从「单调的球体冷却」改写为「至少两个阶段的结构分化」——地球的最深处比我们想象的更复杂。",
