...
一个以浏览器为唯一交付方式的**知识平台**：用可视化、沉浸式的方式探索 22 个知识领域，从可见宇宙下钻到地球，从远古文明走到当代前沿。我们呈现的不是百科词条，而是人类认识世界的完整过程——包括震撼、疑惑、错误与顿悟。
...
| **生命与心灵** | 生命科学与进化 `/life-science` · 医学与公共卫生 `/medicine` · 心理学 `/psychology` · 语言学 `/linguistics` · 教育学与学习科学 `/education` |
...
**跨领域入口**：知识图谱 `/knowledge-graph`（力导向图）· 阅读路线 `/read` · 全站搜索 `/search` · 每日知识 `/daily` · 奇趣知识 `/curiosities` · 分子图鉴 `/molecules` · 知识汇流 `/knowledge-confluence` · 研究前沿 `/<领域>/frontier`（22 域，"正在发生的 2020s 知识"，带年份与来源）。

**当前规模**：约 **2828 篇内容**、搜索索引 2987 个文档、知识图谱 3213 节点。
...
pnpm prepush         # 本地 quality 门禁
pnpm predeploy       # push main 前：CI quality + build 本地全集
```
...
```bash
pnpm gen-all   # 11 步生成链：各域搜索镜像 → 历史路由数据 → wiki-link/反链 → 搜索索引 → 搜索 Worker → 内容图像
```
