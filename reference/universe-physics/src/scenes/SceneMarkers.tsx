...
import { TIER_MARKER_KIND, type MarkerKind, type UniverseTierId } from "@/lib/tier";
...
function MarkerPoint({
  ...
  const markerPos = useMemo(() => marker.position as [number, number, number], [marker.position]);
  ...
  useFrame((_, dt) => {
    ...
    const converged = Math.abs(next - prev) < 0.001;
    glowRef.current = converged ? prev : next;
