...
其中**米尔格若姆加速度** $a_0 \approx 1.2 \times 10^{-10}$ m/s$^2$（相当于每年速度变化约 $3.8 \times 10^{-3}$ m/s，是一个极其微小的加速度尺度）是 MOND 的关键参数。一个常被提及的"巧合"是 $2\pi a_0 \approx c H_0$，即 $a_0$ 在数量级上与哈勃尺度 $cH_0$ 相当，这暗示 MOND 加速度尺度或许与宇宙学背景有深层联系。
