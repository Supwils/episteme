...
五篇论文证明的是无分歧、特征零曲线上的范畴等价。带穿孔的黎曼面、函数域上的更精细算术版本、以及真正的数域朗兰兹，都不被这八百页自动带走。Quanta 所谓纪念碑，指的是几何这一栏被凿开。凿开一块不等于罗塞塔石碑另外两栏已经可读。

范畴等价携带高阶同伦信息，不是两张集合之间的双射，更不是可以随便对译的词典。词典允许意义漂移。等价不允许。

验证也不是一个人读完八百页。接缝藏在 Gaitsgory 先前十年的基础设施与五篇分工之间。接缝处的错误不会在各自熟悉的核心段落里最先暴露。

形式化核验可以把信任从"某位先驱说最美"转到一个小得多的检查内核。内核尚未吞下这套证明时，共同体接受的是分工审读的总和，不是已经机器盖章。

无分歧意味着局部系统在曲线上没有穿刺。物理图像里，这相当于没有插入算符的源。一旦允许穿刺，谱侧的模空间立刻更奇异，现有 Ind-coherent 层的控制还不够把等价再写一遍。这不是附录，而是下一座山。

特征零把系数放在复数上。函数域要把同样的故事搬到有限域上的曲线。Drinfeld 与 Laurent Lafforgue 已经在函数域的经典朗兰兹走得很远。几何证明不会自动给出那次翻译——翻译是另一组论文的工作，不是这五篇的推论。
