...
- **[[proof|证明]]**：信任链条被换掉了：过去是"专家读懂并背书"，现在是"内核检验，加一份人写的形式化陈述"。**风险随之转移到翻译上**——机器保证的是这份形式化命题被证明了，至于它是否就是你想证的那条数学命题，仍需人来核对，且这一步无法自动化。
- **[[large-language-models|大语言模型]]**：模型在形式化环境里搜索证明时，每一步都被即时判定合法与否。**这提供了一种罕见的、不会被幻觉污染的反馈信号**：猜错就被拒绝，猜对就是严格的证明。它也划出了能力边界——环境之外的自然语言表述并没有这种校验。
- **[[computational-linguistics|计算语言学]]**：自动形式化本质是语义解析，难点在于数学写作大量依赖读者补全的省略。**模型必须先猜作者省了什么，再把猜测写死成形式命题**，猜错时形式命题可能仍然合法、仍能被证明，只是证的不是原意——这类失败比语法错误更难被发现。
- **[[metacognition-training|元认知训练]]**：形式化最常见的收获不是发现别人的错，而是发现自己推理里的模糊处：编译不过往往说明那一步本来就没想清楚。**这把隐含的认知负担外显化**，也解释了为什么参与过形式化的人会说自己对证明的理解变深了。
- **[[how-does-science-progress|科学如何进步]]**：一门学科"什么算成立"的标准一旦改变，研究实践会跟着改变。**这里提供了一个正在发生、可被观察的样本**：若可机器核验成为可信度的一部分，选题、协作方式与训练体系都会随之调整，而这类标准变迁通常只能事后重建。
