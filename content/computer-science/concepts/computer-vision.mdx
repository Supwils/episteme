...
分割比分类更"贵"：像素级标注成本远高于一张图一个标签，因此医学、遥感等小数据场景长期依赖 U-Net 这类强归纳偏置结构，而开放世界分割则要等 SAM 用可提示范式把标注负担转嫁给交互。工程上还要分清口径——语义分割不区分同类别多实例，实例分割要求每个物体单独掩码，全景分割（panoptic）则把两者并表；论文里报 mIoU 时必须写清是哪一种，否则数字不可比。
...
生成侧的真正分水岭不只是画质，而是**可控性**：早期 GAN 难调、模式崩塌常见；扩散把生成拆成可逐步条件化的去噪链，才让"文字—图像—编辑—局部重绘"成为同一条流水线。代价是推理步数多、算力贵，于是出现蒸馏、一致性模型等加速路线——工业部署往往在画质、延迟与版权合规之间折中，而不是只追 FID。
...
补一句部署现实：纯 ViT 在小数据上常输给 CNN，因为缺少平移等变与局部性先验；Swin 等层级窗口注意力、以及把卷积归纳偏置注回 Transformer 的混合体，正是在补这一课。MAE 一类掩码自监督则证明：视觉也可以像语言模型那样靠"填空"学表征，从而少依赖昂贵标注——这与后面 CLIP/SAM 的规模化预训练是同一条轴上的不同刻度。
...
还有一层常被技术报告略过的**部署闭环**：视觉系统一旦接入安防、招聘筛查或边境管控，误报不只是统计误差，而会转化为人身限制与污名积累；反过来，被错误标签过的群体更难进入"干净"训练数据，偏见自我强化。因此"准确率提升" alone 不能当作放行许可——必须同时报告分层错误率、阈值校准与人工复核入口，并把拒识（abstain）设计成一等公民，而不是事后补丁。

## 跨域连接

- **[[ai-interpretability|人工智能可解释性]]**：对抗样本证明网络用的判据不是人类用的判据——肉眼看不出的扰动就能翻转高置信度的结论。这把"它准不准"和"它凭什么"分成两个问题，后者正是可解释性研究要回答的：它究竟看了哪里。推论是：提高干净样本上的准确率，不自动提高被扰动时的稳健性，两者常此消彼长。
- **[[gestalt|格式塔心理学]]**：人类视觉先分组再识别，靠闭合、连续与共同命运把场景切成物体，因此对遮挡和低对比度极其稳健。卷积网络没有这层显式分组，判断更依赖纹理统计，于是整幅图叠一层高频噪声就可能被整体改判。
- **[[screening-and-early-detection|筛查与早期发现]]**：在低患病率人群中，即便敏感度与特异度都很高，阳性预测值仍可能很低。这条贝叶斯算术直接适用于视觉系统：报告"准确率极高"却不交代基率与阈值口径，在筛查式部署里几乎不含信息。推论是：同一个模型换个部署人群，阳性预测值就会整体漂移。
- **[[race-and-ethnicity|种族与族群]]**：总体准确率按人数加权，因此可以在某个小群体上任意糟糕而不被察觉。按肤色与性别交叉分层报告之所以关键，正是要让被平均掩盖的差异显形——测什么，决定了哪些错误会被发现。这也说明基准数据集的构成本身，是一项需要被审计的工程决策。
- **[[multilingual-ai|多语言人工智能]]**：把类别名写成一句话，识别就从封闭类别表变成开放词表。代价是判定边界随语言表述漂移：同一个物体换个措辞、换种语言，可能落进另一个类别，模型的"类别"因此不再是稳定对象。推论是：零样本能力的评测必须连提示词一起报告，否则根本不可复现。
