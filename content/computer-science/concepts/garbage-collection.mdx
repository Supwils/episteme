...
Go 语言从 1.5 版本起改用并发三色标记清除算法，此后几个版本持续优化，到 1.8 之后已把 STW 停顿压到亚毫秒级。Java G1 GC（Garbage First）把堆分成等大的 Region，优先回收垃圾最多的 Region，降低停顿时间。Java ZGC 和 Shenandoah 把 STW 时间进一步压缩到 1ms 以下。
