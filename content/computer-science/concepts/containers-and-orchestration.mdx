...
调度与弹性是编排真正发挥作用的地方。调度器根据 CPU、内存、亲和与反亲和、污点与容忍度，把 Pod 放到合适的节点；水平 Pod 自动扩缩（HPA）按指标增减副本，集群自动扩缩再决定要不要增减节点。亲和规则能把有状态副本打散到不同可用区，也能把需要本地缓存的工作负载钉在同一机架——这些约束写进声明后，由控制平面持续执行，而不是靠运维半夜改脚本。

网络与服务发现同样属于编排层。每个 Pod 有独立 IP，Service 用标签选择器给出稳定的虚拟入口；Ingress 或 Gateway API 再把外部流量接到集群内。CNI 插件负责把内核网络接到策略层，NetworkPolicy 才能真正挡住东西向乱连。少了这一层，容器只是"能跑"，谈不上"能管"。
...
调试与变更也要按平台习惯来。临时调试容器、滚动更新的 maxUnavailable、金丝雀与蓝绿发布，都是在缩短平均恢复时间，而不是炫技。声明式的好处是回滚有目标状态可对；坏处是错误的声明也会被忠实地放大到全集群。因此变更评审看的是 Diff 与准入策略，而不只是"镜像能不能 pull 下来"。
...
供应链与准入控制是生产加固的后半段。镜像签名、SBOM、漏洞扫描结果，需要在 Admission Webhook 或策略引擎里变成"能不能进集群"的硬门，而不是散落在 Wiki 里的建议。最小权限的服务账号、只读根文件系统、seccomp 配置文件，最好做成基线模板：新服务默认继承，例外必须显式申请。平台越省事，默认值就越像政策。

多租户与配额把"能跑容器"升级成"能共享集群"。ResourceQuota 限制命名空间能吃多少 CPU 与内存，LimitRange 约束单个容器的默认请求与上限，PriorityClass 决定抢占时谁先牺牲。没有这些，一个失控的批处理任务就能把在线服务饿死。租户隔离还可以叠上网络策略与独立的存储类：同一控制平面服务多个团队时，边界必须写进对象，而不是写进口头约定。

镜像构建本身也在演进。多阶段构建把编译工具留在中间层，最终镜像只带走运行所需；发行版无关的基础镜像进一步缩小攻击面。构建缓存与远程缓存让 CI 不必每次从零开始，但缓存命中策略一错，就会把过期依赖悄悄带进生产。编排解决的是运行期收敛，构建流水线解决的是"你到底装了什么进去"——两边脱节，供应链审计就会断档。

运维文化也会被编排改写。值班手册从「登录某台机器改配置」，变成「看事件、看控制器、看期望与实际的差值」。事故复盘常落在：探针是否诚实、资源请求是否低估、滚动策略是否过猛、密钥轮换是否自动化。平台工程的价值，在于把这些反复踩过的坑收成默认脚手架，让业务团队少碰控制平面的边角细节，同时仍能在声明里表达自己真正需要的约束。

此外，本地开发与生产集群的差距仍是日常摩擦源：开发者用轻量运行时快速迭代，生产却依赖完整的调度、网络策略与密钥注入。把差异显式列成清单（环境变量来源、存储类、服务网格是否注入），比假装「两边完全一样」更有助于减少上线惊喜。
...
- **[[observability-and-monitoring|可观测性与监控]]**：容器寿命短、名字会变、节点会漂移，靠主机名定位问题的老办法因此失效。必须先有统一的标签与追踪上下文，才能把一次请求从入口、网格、服务到数据库重新拼起来，否则事故现场只剩碎片。推论是：把日志与指标的标签体系留到出事后再补，基本等于没有。
- **[[state-capacity|国家能力]]**：声明式配置的关键不是写配置文件，而是控制器能持续观测实际状态并与期望比较。只声明目标却测不到现状，收敛就无从发生。治理同理：看不见的东西无法被调控，可观测性是执行能力的前提而非附属品。推论是：控制平面的价值不在自动化本身，而在它把状态差异变成了可比较的量。
- **[[nudge-libertarian-paternalism|助推与自由家长制]]**：默认值就是事实政策。容器默认以超级用户运行、网络策略默认全通，于是多数集群的真实风险由默认值而非文档决定。可检验推论：把安全基线做成默认配置，比写进规范有效得多。
- **[[institutional-economics|制度经济学]]**：互相竞争的厂商把镜像与运行时规范交给中立组织托管，是为了避免格式碎片化抬高各方的对接成本。收益是镜像到处能跑，代价是创新被锁在规范的演进节奏上，谁也不能单方面改格式。推论是：标准化收益越大的领域，越难出现颠覆性的替代格式。
- **[[ship-of-theseus|忒修斯之船]]**：滚动发布不修补实例，而是整体替换。"同一个服务"的身份因此由声明维持，不再由任何一个进程的连续存在维持——这既是可回滚的前提，也意味着写在容器内的状态默认会被丢掉。推论是：有状态服务上容器平台，难点从来不是启动，而是数据放在哪里。
