...
限流阈值本身也是契约的一部分：过松等于没有保护，过紧会把正常突发流量误判成攻击；对第三方开发者平台，按密钥、按 IP、按终端用户分层配额，往往比单一全局阈值更公平。配额耗尽时的响应也应稳定：同一错误码、可解析的重试建议、以及尽量不改变成功路径上的字段形状，避免客户端把限流当成「偶发解析失败」去瞎重试。

**错误信息的设计**：错误响应应该告诉客户端**为什么**失败，以及**如何修复**（如果可能）。好的错误响应包含：HTTP 状态码、机器可读的错误代码（如 `INVALID_EMAIL_FORMAT`）、人类可读的错误消息、指向文档的链接（可选）。

把「机器可读代码」与「人类可读说明」拆开，还能让客户端按错误码做分支，而文案可以按语言或产品语境替换，不至于为改一句提示就发一版破坏性变更。
...
契约测试（把规范当作双方共同的断言）能把「文档漂移」从口头提醒变成流水线红灯；反过来，若规范只是事后补写的说明书，所谓 API-First 很快退化成「实现优先、文档凑合」。公开 API 还要预设弃用窗口：先标废弃、再给迁移期、最后才移除——窗口长短取决于调用方的多样性和更新节奏，而不是发布日历上的方便。
