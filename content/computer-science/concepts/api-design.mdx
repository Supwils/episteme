...
这不意味着 Level 2 是错的——它在工程上简单、好用、够好。但要明白：**"REST"在日常语境里已经漂移成"资源风格的 HTTP+JSON 接口"，和 Fielding 论文里的严格定义并不是一回事**。知道这层区别，能让你在读规范、和较真的人讨论时不至于各说各话。实务上更有用的提醒是：对外公开接口多数只需做到「资源清晰、方法语义正确、错误可机读」——也就是成熟度二级；硬要为每一条业务路径塞进超媒体链接，往往会换来客户端实现成本陡增、而收益却集中在少数需要动态导航的场景。团队应把「是否追求三级」写成明确的产品决策，而不是默认把论文定义当成工程 KPI。
...
此外，把业务逻辑塞进可组合查询里，容易让「接口契约」变成一团不断生长的图：字段一多，破坏性变更更难察觉。实务上常以持久化查询（persisted queries）、深度/复杂度上限与字段级授权，把客户端自由度压回可控范围。选 GraphQL 的判据应是「数据形状多变且客户端多」，而不是「想显得更现代」——若资源边界清晰、读多写少且缓存价值高，REST 往往更省心。另一条常见教训是：把「任意可组合字段」开放给第三方后，很难再收回——字段一旦被报表、SDK 或合作方脚本写死，删除或改名就变成破坏性变更。因此公开 GraphQL 时更应默认走白名单式持久化查询，把探索性任意查询留在受控的内部工具里。
...
与 REST/JSON 相比，gRPC 的代价也更具体：浏览器不能直接当一等公民（需 gRPC-Web 代理）；调试时二进制载荷不如 JSON 直观；跨组织边界时，对方是否愿意引入同一套 IDL 工具链往往比性能更决定成败。因此它更常落在「你能控制两端」的内部网格，而不是公开的开发者平台。超时、截止时间（deadline）与重试策略应写进服务约定，而不是留给每个调用方各自猜测——否则分布式谬误会借「看起来像本地函数」的假象卷土重来。工程清单上还应写清：哪些 RPC 允许自动重试、哪些必须配合幂等令牌、流式调用中途断连时进度如何恢复。缺少这些约定时，两端各自「看起来合理」的默认值，会在故障高峰叠成雪崩。
...
**限流（Rate Limiting）**：保护 API 不被单一客户端过度消耗。常用算法：令牌桶（Token Bucket）、漏桶（Leaky Bucket）、滑动窗口。响应头（`X-RateLimit-Limit`、`X-RateLimit-Remaining`、`Retry-After`）告知客户端限制状态。 限流阈值本身也是契约的一部分：过松等于没有保护，过紧会把正常突发流量误判成攻击；对第三方开发者平台，按密钥、按 IP、按终端用户分层配额，往往比单一全局阈值更公平。配额耗尽时的响应也应稳定：同一错误码、可解析的重试建议、以及尽量不改变成功路径上的字段形状，避免客户端把限流当成「偶发解析失败」去瞎重试。

**错误信息的设计**：错误响应应该告诉客户端**为什么**失败，以及**如何修复**（如果可能）。好的错误响应包含：HTTP 状态码、机器可读的错误代码（如 `INVALID_EMAIL_FORMAT`）、人类可读的错误消息、指向文档的链接（可选）。 把「机器可读代码」与「人类可读说明」拆开，还能让客户端按错误码做分支，而文案可以按语言或产品语境替换，不至于为改一句提示就发一版破坏性变更。
...
**API 优先（API-First）设计**：先设计 API（写 OpenAPI 规范），再实现。与先实现后文档化相对。API-First 使得前后端可以并行开发（前端基于规范 mock 数据），但需要在设计阶段做更多决策。 契约测试（把规范当作双方共同的断言）能把「文档漂移」从口头提醒变成流水线红灯；反过来，若规范只是事后补写的说明书，所谓 API-First 很快退化成「实现优先、文档凑合」。公开 API 还要预设弃用窗口：先标废弃、再给迁移期、最后才移除——窗口长短取决于调用方的多样性和更新节奏，而不是发布日历上的方便。

## 跨域连接

- **[[distributed-systems|分布式系统]]**：客户端超时后无法区分"请求没到"与"到了但响应丢了"，这条不确定性无法靠重试消除，只能靠语义化解——把操作做成幂等，或给它一把幂等键，让服务端对同一把键只执行一次。所以重试策略必须与接口语义一起设计，而不能留给调用方各自拿捏。
- **[[ring|环论]]**：幂等不是约定而是代数性质：满足自我复合不变的操作，重复施加不改变状态，正如环里的幂等元。由此可判定哪些方法能无条件重试——完整替换与删除可以，创建一笔支付不行，除非外挂一把键把它变幂等。这也解释了幂等键为何该由客户端生成——只有它知道两次请求是不是同一个意图。
- **[[coase-theorem|科斯定理]]**：企业边界由内部协调成本决定。那道禁止一切非接口通信的备忘录，等于强行把内部协调改成市场式交易，于是内部服务与可外销产品之间不再有技术差别——云业务正是这条推论的产物。推论是：一个组织内部的接口质量，长期看会决定它能不能把内部能力变成对外产品。
- **[[languages-change|语言如何变化]]**：语言规范总是追认既成用法，而非相反。工程上的对应版本是：使用者足够多时，文档没承诺的行为也会被依赖，改动它就是破坏性变更。可检验推论：破坏成本随使用者规模超线性增长。
- **[[rule-of-law|法治]]**：向后兼容对应法不溯及既往，版本化对应过渡条款。把账号钉在首次调用时的版本上，就是让存量主体继续适用旧规则，从而把"不许破坏"这条难以执行的禁令，换成由用户自主掌握节奏的迁移。代价是同时维护多份行为，版本越多，回归测试的组合就越大。长期看，真正昂贵的不是「多一个版本号」，而是并行行为分叉后，缺陷只在某一个钉住版本上复现——排障与回放成本会随版本矩阵近似线性甚至超线性上升，因此弃用时间表必须写进契约，而不是靠口头约定。
