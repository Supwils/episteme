...
Bayer 与 McCreight 当时在波音科学实验室工作。论文先在实验室内部流传，一九七二年才正式刊于该刊创刊卷第三期。

两人从未说明字母 B 代表什么。波音、平衡、宽阔，都只是后人猜测。能确定的只有一点：它绝不是每个节点只有两个分叉的二叉树。每个节点被做成一整个磁盘页那么宽，好让一次读取带上尽可能多的键。

Douglas Comer 在一九七九年的综述里把它称作「无处不在的 B 树」。那时距离正式发表不过七年，关系数据库才刚走出实验室。半个世纪后，InnoDB 与 PostgreSQL 的默认索引仍是它的直系后裔。这不是因为没有人试过别的树，而是因为它与块设备的读写粒度绑得太紧：比较可以很便宜，把一页从磁盘搬进 [[memory-hierarchy-caching|内存层次]] 才贵。
...
B 树允许内节点也存放完整记录，B+ 树则把记录全部赶到叶子。内节点因此只剩下分隔键与子页指针，同一页能放下更多路。范围扫描也不必在树里跳来跳去，顺着叶子链表走即可。关系数据库几乎一律选 B+ 变体，不是审美偏好，而是让扇出与范围扫描同时成立。
...
选择性说的是一个谓词大约会留下表中多少比例的行。取值几乎只有两三种的列，选择性极低。优化器往往会判断：跟着这样的索引东一块西一块回表，还不如顺序读完一张表。

覆盖索引把「要不要回表」从必经之路变成可选路径。查询用到的列若全部落在索引里，引擎就不必再去堆表取行。PostgreSQL 把这种路径叫做仅索引扫描。

即便如此，它仍可能要核对可见性。在 [[databases-transactions|多版本并发控制]] 下，二级索引通常并不携带事务可见信息。覆盖并不是魔法，它只是把大量随机回表，收成更少的页访问。
...
System R 是国际商用机器公司圣何塞实验室在一九七四年到一九七九年做的原型。结构化查询语言从这里走出实验室。查询优化也第一次被写成可以发表的算法，而不再是工程师口头相传的经验。

Selinger 那篇路径选择论文做了两件至今仍在用的事。其一是用目录统计估计每条访问路径的代价：顺序扫描、索引等值、索引范围，各自对应不同的页读取次数。其二是用动态规划枚举连接顺序，并把「结果是否已按某列有序」当作可传递的性质。

后一步很关键：有序可以让随后的归并连接或去重少做一次排序。优化器因此不是在比较孤立的算子，而是在比较带着性质的整份计划。

代价模型依赖的选择性，来自行数、不同值个数，以及后来补上的直方图。这些数字一旦过期，计划选择就会跟着偏。System R 把「怎么取」从程序员手里拿走，同时把对统计是否可信的依赖，写进了系统的中枢。
...
执行计划是优化器对自己判断的书面说明。它列出每一步打算走哪条访问路径、估计会碰到多少行。真正跑出来的行数往往和估计对不上。读计划时最值得盯住的正是这种偏差：估计一旦偏了，后面的连接方式、工作内存、是否物化，都会跟着错。

## 慢查询的常见原因

- **未使用索引**：条件列没有索引，或索引被函数/表达式使该列失效（如 `WHERE YEAR(created_at) = 2024` 无法用 `created_at` 上的 B+ 树索引）
- **隐式类型转换**：列是字符串类型却用数字去比较（如 `phone` 是 VARCHAR，却写 `WHERE phone = 13800138000`），数据库会对整列做类型转换，等同于给列套了函数，索引照样失效——这是最隐蔽、也最常被忽视的一种
...
这些条目看起来像故障清单，背后却是同一件事。优化器只能看见自己被允许看见的谓词与统计。函数包住列、类型被悄悄转换、谓词跳过最左前缀，都会让索引从计划空间里消失。引擎并不是拒绝使用索引，而是在它看来，那条索引已经不再是合法路径。
...
选择性与覆盖常常互相牵扯。一个选择性很高的单列索引，点查很快，却几乎总会回表。把更多列塞进索引可以提高覆盖率，同时让叶子更宽、扇出下降、缓存命中变差。于是「要不要覆盖」不是一条绝对规则，而是在回表次数与索引宽度之间的再一次权衡。
...
- **[[b-trees-and-lsm-trees|存储引擎]]**：索引结构的选择先于优化器。就地更新的树读路径短而可预测，追加合并的结构写吞吐高却要查多层，于是同一条查询在两种引擎上的最优计划并不相同——"最好的索引"只对某个读写比例成立。推论是：换引擎等于换掉整套代价假设，原来的索引设计不一定还成立。
- **[[statistics|统计学]]**：基数估计是用直方图与抽样去猜中间结果有多少行，而多表连接时误差会相乘而非相加。可检验推论：优化器最薄弱的一环是估计而非搜索，改进枚举算法收益有限，误差会随参与连接的表增多成倍放大。这也意味着统计信息过期造成的伤害，往往比少建一个索引更严重。
- **[[ensemble-prediction-and-decision|集合预报与决策]]**：气象学早已放弃单一点估计，改用一组成员刻画不确定性再据分布决策。查询优化器却仍靠一个点估计选定计划，估错就满盘皆输——这正是运行时反馈与自适应执行成为当前方向的原因。
- **[[semantics|语义学]]**：声明式查询要求语义与求值策略解耦：说清"要什么"，把"怎么取"留给引擎。可检验推论：一旦某个构造的结果依赖求值顺序，优化器就失去重写自由——所谓"这条语句优化不了"，往往是语义先出了问题。同理，把逻辑塞进标量函数或触发器，等于亲手取消了优化器的重写空间。
- **[[clinical-diagnosis|临床诊断]]**：执行计划相当于检查报告：先看引擎实际选了什么路径、估了多少行，再判断是估计偏了、索引失效还是回表太贵。不看计划直接加索引，与不做检查直接开药同理——指标或许改善，病因并未触及。推论是：性能问题的第一手证据是实际执行统计，而不是预估行数。
...
- Comer, D. "The Ubiquitous B-Tree." *ACM Computing Surveys*, vol. 11, no. 2 (1979): 121–137.
- Chamberlin, D. D. et al. "A History and Evaluation of System R." *Communications of the ACM*, vol. 24, no. 10 (1981): 632–646.
