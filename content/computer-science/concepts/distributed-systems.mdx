...
这八条听起来都像常识，在分布式环境里却全部为假。几乎每一起分布式系统的事故，追根溯源都能落到其中某一条假设被悄悄当成了前提——比如默认"网络是可靠的"，于是没给跨机房调用加超时和重试。
...
需要破除一个流行的误读：CAP 不是让你"三选二"地永久放弃某个属性。Brewer 本人在 2012 年的反思文章里澄清，网络分区其实很罕见，系统在绝大多数时间里既能保证一致性也能保证可用性。

C 与 A 之间的取舍，只在分区真正发生的那一小段时间里才被迫做出。而且这种取舍可以细到按操作、按子系统分别决定，并不是一个全局的二元开关。
...
TrueTime 不返回一个时间点，而是返回一个保证包含真实时间的区间 [最早, 最晚]，其不确定度 $\varepsilon$ 通常被控制在几毫秒以内——相比之下，普通 NTP 校时的误差可达上百毫秒。Spanner 在提交事务时会主动"等掉"这段不确定度（commit wait），以此在全球范围实现外部一致性（也就是线性一致性）。
...
- **Paxos**（Lamport，论文 1998 年发表）：第一个被严格证明正确的实用共识协议，理论上严格，但极难实现
...
2PC 的致命弱点是**阻塞**：如果协调者在发出第二阶段指令前宕机，已经回答"能提交"的参与者只能死等——它既不敢提交，也不敢回滚，被改动的资源一直被锁住。

Jim Gray 与 Lamport 在 2006 年的《Consensus on Transaction Commit》中给出了修补思路：用 Paxos 共识替代单点协调者，只要多数协调者存活就不会阻塞。这也揭示了一层更深的联系——原子提交本质上是共识问题的一个特例。

### 最终一致性

在 AP 系统中，常见的一致性模型是**最终一致性（Eventual Consistency）**：系统保证，如果不再有新的更新，最终所有副本都会收敛到相同的值。

Amazon Dynamo 论文（DeCandia et al., 2007）将最终一致性带入工程主流，并引入了具体的冲突解决机制（向量时钟 + 版本协调）。

需要分清的是：这篇论文描述的 Dynamo 是亚马逊内部使用的无主存储系统，与 2012 年才推出的公有云产品 DynamoDB 并非同一个系统——后者继承了 Dynamo 的部分理念，却改用了单主复制。两者名字相近，常被混为一谈。
