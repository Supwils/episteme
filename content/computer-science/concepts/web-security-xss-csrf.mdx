...
CORS 常被拿来和 CSRF 混为一谈。跨源资源共享管的是浏览器是否允许脚本**读**另一个源的响应；CSRF 管的是浏览器是否会**带着 Cookie 发出**那个请求。读被挡住，写仍然可能发生。把 CORS 配宽松，解决不了伪造转账；把 SameSite 设严，也替代不了输出编码。工具箱里每一件只挡一类越界。
