...
**随机数的关键性**：RSA 密钥生成依赖高质量随机数。2012 年，研究人员分析了 700 万个公开 RSA 公钥，发现约 2 个公钥共享一个质因数——可能是嵌入式设备随机数生成器质量差导致的。共享质因数意味着用 GCD 算法可以在秒内分解这些密钥（Lenstra et al., 2012）。"实现比数学先出问题"的典型案例。
