...
论文最初不被理解（兰波特用考古寓言包装，评审与读者都觉得费解），他将其搁置多年。直到 1998 年才正式发表于 **ACM Transactions on Computer Systems（TOCS）** 16(2)。随后他写了更易读的版本《Paxos Made Simple》（2001），该版本至今是分布式系统课程的必读材料。
