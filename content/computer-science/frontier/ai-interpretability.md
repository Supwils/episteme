...
2021 年，OpenAI 的研究人员在多模态模型 CLIP 中发现了一个奇特的神经元：它会对"蜘蛛侠"这个概念产生强烈反应——无论给它看蜘蛛侠的照片、一幅蜘蛛侠的素描，还是仅仅写着"spider"的一段文字，它都会激活。这种"同一概念无论以图像、符号还是文字形式出现都响应"的多模态神经元，与神经科学家在人脑中发现的著名"Halle Berry 神经元"惊人地相似。

但同一项研究也揭示了一个更棘手的问题：很多神经元并不"专一"，单个神经元会同时对多个看似毫不相关的概念产生反应。神经网络学到的内部表示，对人类来说是不透明的——不是因为无法观察，而是因为难以理解。**机制可解释性（Mechanistic Interpretability）**这个领域，正在尝试系统地改变这一点。
...
关键进展在 2024 年 5 月：Anthropic 的《Scaling Monosemanticity》（Templeton 等）把这一技术规模化应用到实际产品模型 Claude 3 Sonnet 上，训练了三个不同规模的稀疏自编码器（100 万、400 万、最大约 3400 万个特征）。研究者能够识别"金门大桥特征"（激活后模型在文字生成中更频繁提及旧金山）、"欺骗特征"（在角色扮演上下文中激活），以及一些对 AI 安全有意义的特征（如"助手"特征、与身份认同相关的概念）。
