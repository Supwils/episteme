...
这场论战并没有把"语言从哪来"写成终审判决。乔姆斯基一侧把语法看成先天的计算系统，儿童只需在有限参数上做选择。使用为基础（usage-based）的一侧给出另一张图：托马塞洛（Michael Tomasello）在 2003 年的《构建一种语言》里主张，儿童靠读意图、找模式，从听到的具体构式里逐步抽出抽象槽位，不必假设一部专门的普遍语法。这条线索与语言学里的 [[grammar-theories|构式语法与用法模型]] 是同一场争论的两端。

心理语言学实验并不需要先站队。花园幽径句、词频效应、N400 都显示大脑在毫秒尺度上利用统计规律做预测：这与使用为基础的频率敏感性相合，也不否定内部计算。本文写的是这套计算如何运转，而不是把先天论或经验论写成已经赢了的终点。
...
语义违和并不是唯一会在脑电上留下签名的事故。1992 年，Osterhout 与 Holcomb 在《记忆与语言杂志》上报告：读到花园幽径句里那个拆掉错误分析的词（例如 *The broker persuaded to…* 里的 *to*），脑电在约 600 毫秒处出现一个宽的正波，即 **P600**。最初的解读把它当成句法再分析的指标，与 N400 的语义签名成对：一个问这句话通不通，一个问已经搭好的结构还能不能修。

后来的"语义 P600"打乱了这幅干净图：有些语义上讲不通、句法上却完整的句子也会引出 P600。当代解释更常把两个波看成预测误差在不同时间窗里的表现，而不是两个独立模块的开关。
...
更晚近的替代图景不是再找两座中枢，而是两条通路。希科克与波佩尔（Hickok & Poeppel）2007 年在《自然综述：神经科学》里提出双通路模型：腹侧通路大体双侧，把声音接到意义；背侧通路强烈左偏，把声音接到发音动作。布洛卡区与韦尼克区仍是这张网上的重要节点，但它们对应的是十九世纪尸检能看见的损伤综合征，不是现代功能定位的最终答案。
