...
然而就在白皮书发布前后，布达佩斯-马赛-旺岱（BMW）合作组用另一条路线得到了完全不同的答案。他们用的是**晶格 QCD**（Lattice QCD）——直接在离散化的时空格点上数值求解 QCD 方程，从第一原理算出 HVP，不依赖任何实验输入。BMW 在 2020 年《自然》上发表的结果比数据驱动的值高出约 2σ，如果用 BMW 的值代替，理论-实验张力就从 4σ 以上缩小到约 1.7σ，几乎算不上异常[^bmw]。
