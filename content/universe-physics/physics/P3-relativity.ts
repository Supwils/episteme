...
const content: TierContent = {
  ...
  dataCards: [
    ...
    {
      label: "参考系拖拽",
      latinLabel: "Frame Dragging",
      value: "Ω = 2GJ / (c² r³)",
      hint: "Lense-Thirring · 旋转质量拖拽时空",
    },
    {
      label: "LIGO O4 事件数",
      value: "≥ 200+",
  ...
  narrative: [
    {
      ...
      body: [
        ...
        "GPS 卫星是狭义相对论最日常的验证：卫星以 ~3.9 km/s 运动，SR 预言其时钟每天比地面慢 ~7 μs；但卫星在 ~20,200 km 高度，GR 预言引力红移让时钟每天快 ~45 μs。两项合计每天净快 ~38 μs——如果不修正，定位误差每天累积 ~11 km。你的手机导航之所以精确到米级，是因为每颗卫星的原子钟都在实时做相对论修正。",
        "E = mc² 的完整版是 E² = (pc)² + (mc²)²。对静止粒子 E = mc²，对光子（m=0）E = pc。核反应中质量亏损 Δm 转化为 ~200 MeV/反应的能量（P6），太阳每秒把 400 万吨质量变成光——E = mc² 不是公式，是太阳燃烧的原理。",
      ],
    },
    {
    ...
    },
    {
      ...
      body: [
        ...
        "GW150914 的信号应变 h ~ 10⁻²¹ 意味着 LIGO 的 4 km 干涉臂长度变化仅 ~10⁻¹⁸ m——比质子直径还小 1000 倍。这需要极致的激光稳频、 seismic isolation 和量子压缩光技术。LIGO O4（2023–2025）已累积 200+ 事件，开始用群体统计限制黑洞形成通道和自旋分布。",
        "Frame dragging（参考系拖拽）是 GR 的另一预言：旋转质量拖拽周围的时空一起转动。Gravity Probe B（2011）用陀螺仪在极轨道上测到 Lense-Thirring 效应，与 GR 预言吻合到 19%。地球附近的 frame dragging 极其微弱（~0.05 角秒/年），但黑洞附近的 frame dragging 可以强到让光子轨道也被拖着转——这就是 ergosphere，Penrose 过程从中提取旋转能量。",
        "量子引力仍未完成：弦论、圈量子引力（LQG）、渐近安全引力是主要方向；黑洞信息悖论、AdS/CFT 全息、ER=EPR 是当下最活跃的桥梁。LISA（预计 2035 年发射）将在太空探测 mHz 频段引力波，打开超大质量黑洞并合的观测窗口——这是地面探测器触及不到的频段。",
      ],
    },
    {
      heading: "GR 经过的实验场",
      body: [
        ...
        "强场测试现在由 GW + EHT 主导：GW170817 双中子星并合给出 ΔvGW − c / c < 10⁻¹⁵，把许多修正引力理论一次性扼杀；LIGO 的 O4 (2023-2025) 已经累积 200+ 事件，群体统计开始限制 black-hole 形成通道。",
        "MICROSCOPE 卫星（2022）把等效原理的违反上限推到 η < 2 × 10⁻¹⁵——自由落体的加速度与成分无关到这个精度。这个数字直接限制了所有修改引力理论（如 scalar-tensor 理论）的参数空间。下一代 MICROSCOPE 2 目标是 10⁻¹⁷。",
        "GR 没有一处可量化的偏离 —— 这本身就是悬念，因为我们知道它不可能是终极理论。在 Planck 尺度（~10⁻³⁵ m）它必然要和量子力学融合，但所有低能实验都完美吻合。这种「完美」既是 GR 的胜利，也是量子引力研究者的 frustration——没有实验缺口可以指引方向。",
      ],
    },
    {
      ...
      body: [
        ...
        "解开它的关键是「对称性其实早被打破」：留下者一直在惯性系，而旅行者必须减速、掉头、加速才能回来，这段非惯性段在 Minkowski 图上把他的世界线变成折线。沿世界线积分本征时 τ = ∫ √(1 − v²/c²) dt，折线版总是短于直线版（注意 Minkowski 几何里直线是「最长」），所以年龄差是几何事实而非视角错觉。",
        "1971 年 Hafele-Keating 把铯钟绑上民航飞机绕地球东向、西向各飞一圈，测到与 SR + GR 联合预言吻合到几十纳秒——双生子佯谬不再是脑实验。今天的 GPS 卫星时钟修正就是这个实验的工程版本。",
        "双生子佯谬的深层教训是：在弯曲时空中，两点之间的「最短路径」不再是直觉中的直线，而是测地线。自由落体沿测地线运动，本征时最长；任何加速都会缩短本征时。这意味着引力场中的时钟（做自由落体）走得最快——这是 GPS 引力红移修正的物理根源。",
      ],
    },
    {
      heading: "LIGO O4 · 引力波天文学的新时代",
      body: [
        "LIGO/Virgo/KAGRA 的第四次观测（O4，2023–2025）标志着引力波天文学从「首次探测」走向「常规观测」。O4 期间累积了超过 200 个确认事件，包括双黑洞并合、双中子星并合和黑洞-中子星并合。灵敏度的提升来自多项技术改进：更高功率的激光（~200W）、量子压缩光（频率依赖压缩态）、更好的 seismic isolation、以及更精确的镜面涂层热噪声控制。",
        "GW230529（2024 年公布）是一个特别有趣的事件：一颗 ~3.6 M☉ 的天体与一颗 ~1.4 M☉ 的中子星并合。3.6 M☉ 的伴星质量落在「质量间隙」中——太重不能是中子星，太轻不能是已知的恒星质量黑洞。这个事件为理解中子星和黑洞之间的质量边界提供了关键线索。",
        "引力波天文学的另一个突破是「多信使天文学」的成熟。GW170817（2017）是里程碑：LIGO/Virgo 探测到双中子星并合的引力波后 1.7 秒，Fermi 和 INTEGRAL 卫星探测到伽马射线暴 GRB 170817A，随后全球 70+ 个望远镜观测到光学暂现源 AT 2017gfo。这次事件确认了双中子星并合是短伽马射线暴和 r-过程核合成（重元素如金、铂的起源）的来源，也把引力波传播速度与光速的偏差限制到 < 10⁻¹⁵。",
        "下一步是 LISA（预计 2035 年发射），一个太空引力波探测器，臂长 250 万公里，探测 mHz 频段的引力波。LISA 将打开超大质量黑洞并合（10⁵–10⁹ M☉）、极端质量比旋进（EMRI）和宇宙早期相变产生的随机引力波背景的观测窗口——这些信号在地面上被地震噪声淹没，只有太空才能探测。",
      ],
    },
    {
      heading: "EHT 银河系中心 · Sgr A* 与黑洞阴影",
      body: [
        "事件视界望远镜（EHT）在 2019 年拍摄了 M87* 的第一张黑洞阴影照片后，2022 年 5 月公布了银河系中心超大质量黑洞 Sgr A* 的图像。Sgr A* 的质量约 400 万太阳质量（M87* 约 65 亿），但距离仅 26000 光年（M87* 约 5500 万光年），两者的角直径相似（~50 微角秒），因此用同一套 EHT 基线可以成像。",
        "Sgr A* 的成像比 M87* 困难得多：变化时标仅 ~10 分钟（M87* ~数天），因为黑洞越小、质量越轻，动力学越快。EHT 团队必须在一夜之内采集足够数据，然后从数万张「快照」中平均出稳定的环状结构。最终图像显示一个直径约 52 微角秒的亮环，中心暗影与 Kerr 黑洞的预言吻合到 ~10%。",
        "2024 年 EHT 发布了 Sgr A* 的偏振图像，揭示了黑洞周围的磁场结构——螺旋形的磁场线与 M87* 的喷流形成机制高度一致，暗示即使不喷射强大喷流的 Sgr A* 也有类似的磁场拓扑。这些观测直接约束了黑洞吸积流的物理模型：MAD（magnetically arrested disk）还是 SANE（standard and normal evolution）。",
        "下一代 EHT（ngEHT）计划增加更多天线、更高频段（345 GHz）和太空基线，目标是拍摄「黑洞电影」——实时观测吸积流的轨道运动和耀斑产生过程。结合 LISA 的引力波数据，我们将首次同时用电磁波和引力波「看」同一个黑洞系统——多信使黑洞物理学的终极愿景。",
      ],
    },
    {
      ...
      body: [
        ...
        "这是引力波探测器的物理原理：经过 LIGO 干涉臂的引力波本质上是一阵周期性的潮汐力，把两条臂以 h × L ~ 10⁻¹⁸ m 的振幅相反挤压。应变 h = ΔL / L ~ 10⁻²¹ 意味着 4 km 臂长变化不到质子直径的千分之一——这是人类有史以来最精密的测量。",
        "同一条方程在另一个方向上解释了 Roche 极限：当卫星走到主星 r < 1.26 (ρ_M/ρ_m)^(1/3) R_M 时，潮汐力超过自身引力，卫星被撕成环——土星环、Shoemaker-Levy 9 与黑洞 TDE 都是这条几何句子的不同语调。",
        "潮汐力也是黑洞 spaghettification 的原因：一个自由落向黑洞的人，脚比头感受到更强的引力加速度（因为更靠近奇点）。对恒星质量黑洞（~10 M☉），人在视界外就会被撕碎；对超大质量黑洞（~10⁹ M☉），人可以安然穿过视界——直到撞上奇点。这种「大小依赖」纯粹是曲率标度的几何后果。",
      ],
    },
  ],
  sources: [
    ...
    {
      label:
        "Abbott et al. (LIGO) 2016 — Observation of Gravitational Waves from a Binary Black Hole Merger (GW150914)",
      url: "https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.116.111102",
      kind: "paper",
    },
    {
      label:
    ...
    {
      label:
        "Hafele & Keating 1972 — Around-the-world atomic clocks: Predicted and observed relativistic time gains",
      url: "https://www.science.org/doi/10.1126/science.177.4044.166",
      kind: "paper",
    },
    {
  ...
  discussionQuestions: [
    "GPS 卫星需要实时做相对论修正才能保持米级精度——你能想到其他「隐藏」在日常生活中的相对论效应吗？",
    "黑洞内部的奇点意味着广义相对论在那里失效——你认为「量子引力」会如何「抹平」奇点？",
    "GW170817 引力波事件同时被电磁波探测到——多信使天文学如何改变我们理解宇宙的方式？",
  ],
};
