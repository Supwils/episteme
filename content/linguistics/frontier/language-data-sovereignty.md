...
- **[[corpus-linguistics|语料库语言学]]** 依赖可检索、带标注的语言材料。数据主权并不否定语料研究，而是要求把来源、权限和语境也视为语料质量；缺失治理元数据的数据集并不真正完整。能检索、不能说明谁允许检索，语料就不完整。
- **[[digital-rights-and-privacy|数字权利与隐私]]** 多以个人为权利主体。语言数据常牵涉亲属、群体和未来世代；这迫使隐私框架加入集体影响，同时防止“集体”名义压制个人选择。一个人点了同意，不能代替尚未出生的说话人。
- **[[museum-restitution-digital-repatriation|博物馆返还与数字归还]]** 面对相同的数字副本权力问题。把文件发回社区不等于归还控制；可验证的归还应包含主文件、权限、维护经费和未来版本决定权。副本寄出、主文件仍在机构服务器上，控制权就还没有搬家。
- **[[machine-learning-data|机器学习数据]]** 解释数据血缘、去重和训练集合成。若数据管线不能追踪每批语言资源的来源与许可，模型开发者就无法兑现删除、收益分享或用途限制。血缘断了，删除请求就没有可执行的地址。

- **[[language-documentation-ethics|语言记录伦理]]** 把同意写成可撤回的关系，而不是一次点击。主权框架若不能处理拒绝与再协商，许可就会在采集那天被用尽。不能拒绝的同意，只是一次采集手续。

- **[[corpus-annotation-and-agreement|语料标注与一致性]]** 权限元数据也是标注。不一致的来源字段，会让删除和收益分享在执行时找不到对象。
