...
这场争论还有一条更早的形式语言线索。乔姆斯基在 1956 年把生成装置分成正则、上下文无关、上下文有关与无限制四层，用来标明一类规则系统原则上能生成哪些符号串。神经网络把问题改写成经验问题：在有限样本上训练之后，模型能否把规律推到训练分布之外。

德莱唐等人（Delétang et al., 2023）用受控的形式语言任务比较多种架构，发现循环网络与 Transformer 在非正则任务上的分布外泛化往往失败；带栈或纸带一类结构化记忆的网络，才能在上下文无关与上下文有关任务上稳定外推。这个结果不能直接翻译成“大模型不懂自然语言”——自然语言未必等于形式语言考场上的那几类语言。它提醒我们：层级能力是架构与记忆设计的问题，不是参数量自动送上门的礼物。
...
把争论推进实验室的，是两代预训练模型。2018 年，德夫林等人（Devlin et al.）提出 BERT，用掩码语言模型在双向上下文里预训练编码器，使“打开内部表征看句法”第一次成为常规实验。2020 年，布朗等人（Brown et al.）发表 GPT-3，表明规模足够的自回归模型可以靠少样本提示完成多种语言任务，公众讨论随即从分类器转向生成器。

皮安塔多西主要针对后一类系统发言；乔姆斯基、罗伯茨与瓦图穆尔的回应，针对的也是聊天界面造成的流利错觉。时间线必须分开：句法探针的经典结果大多写在 BERT 上，新一轮刺激贫乏之争则发生在 GPT-3 之后。两者都叫“语言模型”，实验对象并不相同。
...
探针之后更硬的一步是因果干预：不是问“表征里有没有句法信息”，而是问“拿掉或改写这些信息，模型的下一步预测会不会变”。埃拉扎尔等人（Elazar et al., 2021）提出“失忆探针”（amnesic probing），用迭代零空间投影从 BERT 表征里抹去词性或依存一类属性，再看语言模型损失是否上升。他们的要点很尖锐——探针分类器的高分，与该信息是否被下游计算依赖，并不相关。

维格等人（Vig et al., 2020）把因果中介分析引进 Transformer，追问哪些神经元或注意力头介导了特定行为。孟等人（Meng et al., 2022）在 GPT 类模型上定位事实联想，并用秩一编辑改写中层前馈模块。对语言学理论，这些方法的价值不是“证明机器有语法”，而是把“编码”与“使用”拆开：读得出树距，仍需干预才能说模型靠它做预测。
...
- Hewitt, J., & Manning, C. D. “A Structural Probe for Finding Syntax in Word Representations.” *NAACL-HLT*, 2019.
- Elazar, Y., Ravfogel, S., Jacovi, A., & Goldberg, Y. “Amnesic Probing: Behavioral Explanation with Amnesic Counterfactuals.” *Transactions of the Association for Computational Linguistics* 9 (2021): 160–175.
...
- Delétang, G., et al. “Neural Networks and the Chomsky Hierarchy.” *ICLR*, 2023.
