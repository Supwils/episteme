...
- **[[fairness-in-ml|机器学习公平]]** 展示多个统计公平定义及其不可兼得。法律的任务不是选择最优公式，而是把指标选择、错误代价与权利目的公开，使技术优化可被规范质询。指标一旦被写进合规清单，优化就会朝清单跑，未入清单的伤害会被写成噪声。
- **[[discrimination-and-inequality|歧视与不平等]]** 区分直接差别对待、结构性机制和累积优势。模型可能不使用群体标签仍复制历史；社会技术审计因此必须分析数据如何由制度生成。审计若只看输出分数，会把标签、缺失值和历史拒绝已经写好的不平等当成算法发明。
- **[[evidence-and-proof|证据与证明]]** 提供相关性、可采性、证明力和举证责任框架。审计统计不是裁判结论；其样本、误差、对照和专家方法必须允许对方质证，才可能成为可靠法律事实。对方进不了样本框，显著性就还停在报告里。
- **[[experimental-linguistics|实验语言学与因果推断]]** 提醒观察差异不等于因果。反事实测试、自然实验和部署前后比较可以增强推断，但若干预同时变化，审计就必须诚实保留不确定性。人工审核与模型一起改时，增量不能单独记在模型账上。

- **[[causal-inference-experiments-observational-studies|因果推断]]** 审计里的差距是观察差异。没有对照、随机化或清晰的反事实，差距不能直接写成歧视机制已经在模型里被识别。
