...
- **[[digital-rights-and-privacy|数字权利与隐私]]** 提供个人访问、更正、删除与目的限定。跨境场景增加执行距离：权利写在一国法律里，数据和控制者却可能在多国，救济必须依赖合作与可追踪责任链。删除权写得出、副本在别处仍活着，权利就停在纸面。
- **[[cloud-computing|云计算]]** 解释虚拟化、区域、副本和托管服务如何分配控制。法律若只问服务器位置，会漏掉密钥与管理平面；技术架构应把谁实际能访问作为核心证据。区域标签不等于控制权，管理平面在哪，命令才落在哪。
- **[[state-capacity|国家能力]]** 管辖冲突最终要问谁能把命令送达存储与密钥。领土原则仍重要，却不够：稳定方案需要可执行的国际程序，而不是让企业逐案秘密决定哪国命令优先。
- **[[data-governance|数据治理]]** 把分类、血缘、访问与保留变成组织能力。可检验推论是，拥有实时数据流和法律依据地图的机构，应更快识别冲突并减少无目的复制，而非只在事故后追查。地图缺一块，合规就会在复制完成之后才开始。

- **[[institutional-economics|制度经济学]]** 跨境规则改变的是信息与执行成本。企业选择把数据放在哪里，常常是在比较哪一套命令更贵，而不是哪一套权利更完整。成本地图比服务器地图更能预测冲突会在哪一层爆发。
