  ...
  return (
    <ul className="space-y-2" role="list">
      {bookmarks.map((b) => (
        <li
          key={b.slug}
          className="group flex items-start gap-3 rounded-lg border border-white/[0.04] p-3 transition-all hover:border-white/[0.1] hover:bg-white/[0.02]"
        >
          <Link href={b.url} onClick={onClose} className="min-w-0 flex-1">
            ...
            <span className="text-fg-secondary line-clamp-2 text-sm font-medium">{b.title}</span>
          ...
          <button
            ...
            className="flex h-6 w-6 shrink-0 items-center justify-center rounded text-[#9ca3af] opacity-0 transition-all group-focus-within:opacity-100 group-hover:opacity-100 hover:text-red-400"
          >
            <svg
              width="12"
              height="12"
            ...
            >
              <div className="mb-1.5 flex items-center gap-2">
                ...
                {item.scrollPct >= 95 ? (
                  ...
                  <span className="text-fg-disabled font-mono text-[9px]">{item.scrollPct}%</span>
                )}
              </div>
              <p className="text-fg-secondary group-hover:text-fg-primary line-clamp-1 text-sm font-medium transition-colors">
                {item.title}
              </p>
              {item.scrollPct < 95 && (
