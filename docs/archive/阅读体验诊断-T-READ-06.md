...
> **已归档（2026-08-25）**：清单项已由 `T-READ-01`–`09` 消费。不是真相源；新代理不必读。
...
5. **3 域的每日卡只链到域首页（中）**：✅ #279 已修——数学 15 / 生命科学 15 / 宇宙学 12 条全部改链真实文章；经济学「看不见的手」一并改到亚当·斯密页。守卫测试禁止这四份目录再回落到域首页。
6. **「历史上的今天」条目纯展示、零出口（中）**：`components/OnThisDay.tsx` 全文件无 Link/href；同样硬编码白色透明卡片与时间轴线。事件数据里其实有现成 url。
7. **/daily 与 4 个新域彻底无关（低）**：✅ #279 续已修——语言学 / 法学 / 艺术 / 工程各 10 条事实卡接入 `DailySelected` 与领域网格，全部深链到真实文章。

### C. 搜索到阅读

8. **/search 无结果页是死胡同（低）**：✅ #279 已修——无结果时给出 `/read` `/daily` `/random` 三个策展出口；空查询「2200+」改为 `SEARCH_STATS.articles`。

### D. 阅读路线 `/read`

9. **12 条路线只覆盖 9 个域（低）**：✅ 部分已修——新增医学路线「医学如何学会知道」（7 章）。地球科学、化学仍缺。
10. **路线末章「读完 ✓」体验落差（低）**：✅ 已修——末章改为「本路线读完 / 回到目录」，仍链到 `/read/[slug]`，不碰进度持久化。
