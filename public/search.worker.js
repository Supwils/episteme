/* generated by pnpm gen-search-worker; do not edit */

"use strict";
(() => {
  // lib/search/tokenize.ts
  var HAN = /\p{Script=Han}/u;
  var WORD = /[\p{Letter}\p{Number}]/u;
  ...
  // lib/search/artifact.ts
  async function loadArtifact(signal) {
    ...
    return {
      search(query, limit) {
        ...
        for (const result of index.search(normalized)) {
          const doc = artifact.docs[result.id];
          if (!doc) continue;
          scanned.push({
            title: doc.t,
            subtitle: doc.s,
        ...
        const isExact = (hit) => hit.title.trim().toLowerCase() === exact;
