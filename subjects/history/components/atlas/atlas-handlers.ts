...
export function setupEventHandlers(
  ...
  function onMove(e: MouseEvent) {
    ...
    if (state.level === 1) {
      ...
      if (newHover !== state.hoveredEra) {
        state.hoveredEra = newHover;
        needsRedraw = true;
      }
      ...
      if (newHover !== state.hoveredTopic) {
        state.hoveredTopic = newHover;
        needsRedraw = true;
      }
      canvas.style.cursor = topic
        ? "pointer"
        : isNearAnyTopic(state.activeEra, pos.x, pos.y, state.scaleRatio)
          ? "default"
          : "pointer";
  ...
  function onClick(e: MouseEvent) {
    ...
    if (state.level === 1) {
      ...
      else if (!isNearAnyTopic(state.activeEra, pos.x, pos.y, state.scaleRatio))
        returnToRoot(false);
    }
  }
  ...
  function onKeydown(e: KeyboardEvent) {
    ...
    if (e.key === "-") {
      state.zoom = Math.max(0.4, state.zoom * 0.87);
      markDirty();
    }
    if (e.key === "0") {
      zoomTo(1, state.W / 2, state.H / 2);
    }
  }
