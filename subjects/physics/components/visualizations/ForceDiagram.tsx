...
function computeNetForce(forces: Force[]): {
  fx: number;
  fy: number;
  magnitude: number;
  angle: number;
...
export function ForceDiagram({ className }: { className?: string }) {
  ...
  const updateForceMagnitude = useCallback(
    (forceId: string, value: number) => {
      setForces((prev) =>
        prev.map((f) => {
  ...
  return (
    <div className={cn("flex flex-col gap-4", className)}>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-700 bg-gray-900/60">
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className="h-auto w-full" style={{ minHeight: 300 }}>
          <defs>
          ...
          {scenarioId === "incline" && scenario.inclineAngle !== undefined && (
            <InclinePlane
              cx={scenario.objectX}
              cy={scenario.objectY + 30}
              angle={scenario.inclineAngle}
            />
          )}

          {scenarioId === "spring" && <SpringCoil x={cx} y1={cy - 140} y2={cy - 20} />}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="space-y-3 rounded-xl border border-gray-700 bg-gray-900/60 p-4">
          ...
          <div className="border-t border-gray-700 pt-2">
            <Slider label="m" value={mass} min={1} max={50} color="#9ca3af" onChange={setMass} />
          </div>
        </div>
        ...
        <div className="space-y-3 rounded-xl border border-gray-700 bg-gray-900/60 p-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span className="font-medium text-purple-400">合力 |F|</span>
              <span className="font-medium text-purple-300 tabular-nums">
                {netForce.magnitude.toFixed(2)} N
              </span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between border-t border-gray-700 pt-2">
              <span className="font-medium text-purple-400">加速度 a</span>
              <span className="font-medium text-purple-300 tabular-nums">
      ...
      <div className="flex flex-wrap gap-4 text-xs text-gray-500">
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-0.5 w-3 rounded bg-red-500" /> 重力
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-0.5 w-3 rounded bg-blue-500" /> 法向力
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-0.5 w-3 rounded bg-orange-500" /> 摩擦力
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-0.5 w-3 rounded bg-green-500" /> 张力
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-0.5 w-3 rounded bg-purple-500" /> 合力
        </span>
      </div>
    </div>
  );
