...
export function GalileosRamp({ className }: { className?: string }) {
  ...
  const chartDataRef = useRef<{ times: number[]; distances: number[] }>({
    times: [],
    distances: [],
  });
  useEffect(() => {
  ...
  const drawBall = useCallback(
    (ctx: CanvasRenderingContext2D, w: number, h: number, pos: number) => {
      const layout = getLayout(w, h);
    ...
    <div className={cn("flex flex-col gap-4", className)}>
        ...
        <div className="bg-bg-panel/90 absolute top-3 left-3 rounded-lg p-3 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="accent-accent-warm w-24"
              disabled={isPlaying}
            />
            <span className="text-accent-warm w-10 text-right font-mono text-sm">{angle}°</span>
          </div>
        </div>

        {/* Ball position indicator */}
        <div className="bg-bg-panel/90 absolute top-3 right-3 rounded-lg px-3 py-2 backdrop-blur-sm">
          <span className="text-fg-muted text-xs">距离: </span>
          <span className="text-accent-cool font-mono text-sm">{(ballPos * 100).toFixed(1)}%</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="bg-accent-warm/20 text-accent-warm hover:bg-accent-warm/30 rounded-lg px-4 py-2 text-sm transition-colors disabled:opacity-40"
        >
          记录数据
        </button>
        <span className="text-fg-muted ml-auto text-xs">已记录 {measurements.length} 组数据</span>
      </div>

      {/* Measurements table */}
      {measurements.length > 0 && (
        <div className="border-fg-disabled/30 overflow-hidden rounded-lg border">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-fg-disabled/30 bg-bg-panel/50 border-b">
                <th className="text-fg-muted px-3 py-2 text-left">#</th>
                <th className="text-fg-muted px-3 py-2 text-left">角度</th>
                <th className="text-fg-muted px-3 py-2 text-left">时间 (s)</th>
                <th className="text-fg-muted px-3 py-2 text-left">时间² (s²)</th>
                <th className="text-fg-muted px-3 py-2 text-left">距离 (%)</th>
              </tr>
            </thead>
            <tbody>
              {measurements.map((m, i) => (
                <tr key={i} className="border-fg-disabled/20 border-b">
                  <td className="text-fg-muted px-3 py-1.5">{i + 1}</td>
                  <td className="text-fg-secondary px-3 py-1.5">{angle}°</td>
                  <td className="text-accent-cool px-3 py-1.5 font-mono">{m.time.toFixed(3)}</td>
                  <td className="text-accent-cool px-3 py-1.5 font-mono">
                    {(m.time * m.time).toFixed(3)}
                  </td>
                  <td className="text-accent-warm px-3 py-1.5 font-mono">
      ...
      <div className="border-accent-warm/20 bg-accent-warm/5 rounded-lg border p-3">
        <p className="text-fg-secondary text-xs">
          <span className="text-accent-warm font-medium">伽利略发现：</span>
