...
export function CavendishExperiment({ className }: { className?: string }) {
  ...
  const drawWire = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number, twist: number) => {
  ...
  const drawArm = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number, twist: number) => {
    const layout = getLayout(w, h);
    ...
    const smallGrad1 = ctx.createRadialGradient(-layout.armLength, 0, 0, -layout.armLength, 0, layout.smallMassR);
    ...
    const smallGrad2 = ctx.createRadialGradient(layout.armLength, 0, 0, layout.armLength, 0, layout.smallMassR);
  ...
  const drawLargeMasses = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number, angle: number) => {
    const layout = getLayout(w, h);
    ...
    if (isPlaying && phase === 'attracting') {
      ...
      const smallLeft = layout.centerX - layout.armLength * Math.cos(twistRef.current * Math.PI / 180);
      const smallTop = layout.centerY + layout.armLength * Math.sin(twistRef.current * Math.PI / 180) * 0.3;
      ...
      const smallRight = layout.centerX + layout.armLength * Math.cos(twistRef.current * Math.PI / 180);
      const smallBottom = layout.centerY - layout.armLength * Math.sin(twistRef.current * Math.PI / 180) * 0.3;
  ...
  const drawAngleScale = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number, twist: number) => {
    const layout = getLayout(w, h);
  ...
  return (
    <div className={cn('flex flex-col gap-4', className)}>
      <div className="relative aspect-[4/3] w-full overflow-hidden rounded-xl border border-fg-disabled/30 bg-bg-deep">
        ...
        {/* Phase indicator */}
        <div className="absolute left-3 top-3 rounded-lg bg-bg-panel/90 px-3 py-2 backdrop-blur-sm">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={{ opacity: 0, y: -5 }}
            >
              <p className={cn('text-xs font-medium', phaseColors[phase])}>
                {phaseLabels[phase]}
              </p>
        ...
        {/* Measurement readout */}
        <div className="absolute right-3 top-3 rounded-lg bg-bg-panel/90 p-3 backdrop-blur-sm">
          <div className="space-y-1">
            <div>
              <span className="text-[10px] text-fg-muted">细丝扭转角</span>
              <p className="font-mono text-sm text-accent-warm">
                {(wireTwist * 1000).toFixed(2)} × 10⁻³ °
              </p>
            </div>
            <div>
              <span className="text-[10px] text-fg-muted">引力常数 G</span>
              <p className="font-mono text-sm text-accent-cool">
                6.674 × 10⁻¹¹
              </p>
              <p className="text-[10px] text-fg-muted">N·m²/kg²</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-4">
        <div className="flex items-center gap-2 text-xs text-fg-secondary">
          <span className="h-3 w-3 rounded-full bg-accent-cool" />
          小铅球 (m)
        </div>
        <div className="flex items-center gap-2 text-xs text-fg-secondary">
        ...
        <div className="flex items-center gap-2 text-xs text-fg-secondary">
          <span className="inline-block h-px w-4 bg-fg-muted" />
          细金属丝
        </div>
      </div>
      ...
      <div className="rounded-lg border border-accent-warm/20 bg-accent-warm/5 p-3">
        <p className="text-xs text-fg-secondary">
          <span className="font-medium text-accent-warm">卡文迪什的贡献：</span>
