...
export function RutherfordScattering({ className }: { className?: string }) {
  ...
  const drawScene = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number) => {
    ...
    const nucGrad = ctx.createRadialGradient(
      layout.nucleusX, layout.nucleusY, 0,
      layout.nucleusX, layout.nucleusY, layout.nucleusRadius * 3,
    );
  ...
  const render = useCallback(() => {
    ...
    for (const p of particlesRef.current) {
      if (p.trail.length > 1) {
        ctx.strokeStyle = p.scattered
          ? 'rgba(255, 180, 90, 0.3)'
          : 'rgba(106, 208, 255, 0.2)';
        ctx.lineWidth = 1;
        ctx.beginPath();
  ...
  const scatterPercent = stats.total > 0
    ? ((stats.scattered / stats.total) * 100).toFixed(1)
    : '0.0';
    ...
    <div className={cn('flex flex-col gap-4', className)}>
      <div className="relative aspect-[16/9] w-full overflow-hidden rounded-xl border border-fg-disabled/30 bg-bg-deep">
        ...
        <div className="absolute left-3 top-3 rounded-lg bg-bg-panel/90 p-3 backdrop-blur-sm">
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
            ...
            <span className="font-mono text-fg-primary">{stats.total}</span>
            <span className="text-fg-muted">穿过</span>
            <span className="font-mono text-accent-cool">{stats.passed}</span>
            <span className="text-fg-muted">偏转</span>
            <span className="font-mono text-accent-warm">{stats.scattered}</span>
        ...
        <AnimatePresence>
          {stats.total > 20 && (
            <motion.div
              ...
              className="absolute bottom-3 right-3 rounded-lg bg-bg-panel/90 p-3 backdrop-blur-sm"
            >
              <p className="text-xs text-fg-muted">偏转率</p>
              <p className="text-lg font-mono text-accent-warm">{scatterPercent}%</p>
              <p className="text-[10px] text-fg-muted">（实际约 0.01%）</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <div className="rounded-lg border border-accent-warm/20 bg-accent-warm/5 p-3">
        <p className="text-xs text-fg-secondary">
          <span className="font-medium text-accent-warm">卢瑟福的震惊：</span>
          绝大多数α粒子直接穿过金箔，但极少数被大角度反弹。
          卢瑟福说：&ldquo;这就像你用 15 英寸的炮弹射击一张薄纸，炮弹却反弹回来打中了你。&rdquo;
