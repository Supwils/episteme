...
export function InflationCalculator() {
  ...
  const equivalent = useMemo(
    () => calcEquivalent(amount, fromYear, toYear),
  ...
  const lossPercent = useMemo(
    () => ((equivalent - amount) / equivalent) * 100,
    [amount, equivalent]
  );
  ...
  return (
    <div className="w-full">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              min={0}
              step={100}
              className="bg-bg-elevated border-border-faint text-fg-primary focus:border-accent-gold w-full rounded-lg border px-3 py-2 font-mono text-sm transition-colors outline-none"
              aria-label="输入金额"
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
          ...
          <div>
            ...
            <div className="data-value stat-negative">
              $
              {animatedEquiv.toLocaleString("en-US", {
      ...
      <div
        ...
        style={{
          borderColor: "rgba(212,120,80,0.2)",
          background: "linear-gradient(90deg, rgba(212,120,80,0.05) 0%, transparent 60%)",
        }}
      >
        <p className="text-fg-secondary text-sm leading-relaxed">
          ...
          <span className="text-accent-gold font-mono">{fromYear}</span>
          ...
          <span className="text-fg-muted"> 于 </span>
          <span className="text-accent-gold font-mono">{toYear}</span>
      ...
      >
        ...
        {[-4, -2, 0, 2, 4, 6, 8, 10, 12, 14, 16]
          .filter((v) => Math.abs(v) <= chartMaxRate + 1)
          .map((v) => {
            const y = rateToY(v);
            return (
              <g key={v}>
