...
// Module-level cache — skipped in dev to avoid stale data during HMR.
let cachedAllContent: ContentItem[] | null = null;

export function getAllContent(): ContentItem[] {
  if (cachedAllContent && process.env.NODE_ENV === "production") return cachedAllContent;
  cachedAllContent = [
...
function matchRelated(rel: string, item: ContentItem): boolean {
  return rel === item.slug || rel === item.title;
}

export function getRelatedItems(item: ContentItem): ContentItem[] {
  const all = getAllContent();
  const resolved = item.related.map((rel) => all.find((i) => matchRelated(rel, i)));
  const unmatched = item.related.filter((_, i) => !resolved[i]);
  if (unmatched.length > 0) {
    if (process.env.NODE_ENV === "development") {
      console.warn(`[${item.slug}] Unmatched related entries: ${unmatched.join(", ")}`);
    }
  }
  return resolved.filter((i): i is ContentItem => i !== undefined && i.slug !== item.slug);
}
