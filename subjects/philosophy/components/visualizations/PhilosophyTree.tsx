...
export function PhilosophyTree() {
  ...
  const filteredNodes = useMemo(
    () => PHILOSOPHERS.filter((p) => activeTraditions.has(p.tradition) && activeEras.has(p.era)),
  ...
  const filteredEdges = useMemo(
    () => INFLUENCES.filter((e) => filteredNodeIds.has(e.from) && filteredNodeIds.has(e.to)),
    [filteredNodeIds]
  );
    ...
    <div className="flex h-full flex-col">
      <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-1.5">
          {(Object.entries(TRADITION_LABELS) as [Tradition, string][]).map(([t, label]) => {
            ...
            {timelineMarkers.map((marker) => (
              <g key={marker.year}>
                ...
                <text
                  ...
                  fontFamily="monospace"
                >
                  {marker.year < 0 ? `前${Math.abs(marker.year)}` : marker.year.toString()}
                </text>
              </g>
            ))}
              ...
              return (
                <g key={`${edge.from}-${edge.to}`}>
                  <path
                    ...
                    markerEnd={
                      isHighlighted ? "url(#arrow-influence-highlight)" : "url(#arrow-influence)"
                    }
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              <div
                ...
                style={{
                  backgroundColor: TRADITION_COLORS[selectedPhilosopher.tradition],
                }}
              />
              <span
