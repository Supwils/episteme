...
function codonsToProtein(
  codons: string[]
): { codon: string; aminoAcid: string; isStart: boolean; isStop: boolean }[] {
...
const MODE_INFO: Record<ViewMode, { label: string; description: string }> = {
  ...
  transcription: {
    label: "转录",
    description:
      "RNA 聚合酶读取 DNA 模板链（3\'→5\'），合成互补的 mRNA（5\'→3\'）。A→U, T→A, G→C, C→G。",
  },
  translation: {
    label: "翻译",
        ...
        <div className="p-4">
          <div className="mb-4">
            ...
            <input
              ...
              className="border-border-subtle bg-bg-deep text-fg-primary focus:border-accent-cool/40 w-full border px-3 py-2 font-mono text-sm outline-none"
          ...
          <AnimatePresence mode="wait">
            {mode === "overview" && (
              ...
              >
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-1 flex-wrap gap-0.5">
                      {dna.split("").map((b, i) => (
                        <span
                          key={`dna-${i}`}
                          className="border-border-faint inline-flex h-6 w-5 items-center justify-center border font-mono text-[10px] font-bold"
                          style={{
                            color: BASE_COLORS[b] ?? "#888",
                            borderColor: `${BASE_COLORS[b] ?? "#888"}30`,
                          }}
                        >
                          {b}
                        </span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-1 flex-wrap gap-0.5">
                      {complementStrand.split("").map((b, i) => (
                        <span
                          key={`comp-${i}`}
                          className="border-border-faint text-fg-muted inline-flex h-6 w-5 items-center justify-center border font-mono text-[10px] font-bold"
                          style={{
                            color: BASE_COLORS[b] ?? "#888",
                            borderColor: `${BASE_COLORS[b] ?? "#888"}20`,
                          }}
                        >
                          {b}
                        </span>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-accent-cool w-16 font-mono text-[10px]">mRNA</span>
                    <div className="flex flex-1 flex-wrap gap-0.5">
                      {mrna.split("").map((b, i) => (
                        <span
                          key={`mrna-${i}`}
                          ...
                          style={{
                            color: BASE_COLORS[b] ?? "#888",
                            borderColor: `${BASE_COLORS[b] ?? "#888"}40`,
                            background: `${BASE_COLORS[b] ?? "#888"}10`,
                          }}
                        >
                          {b}
                        </span>
                ...
                <div className="border-border-faint border-t pt-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {protein.length === 0 ? (
                      <span className="text-fg-disabled font-mono text-xs">
                        未找到起始密码子 AUG
                      </span>
                    ) : (
                      protein.map((p, i) => (
                        ...
                        >
                          <span className="text-fg-muted text-[8px]">{p.codon}</span>
              ...
              >
                    ...
                    .map((b, i) => (
                      <g key={`trans-dna-${i}`}>
                        <rect
                          ...
                          height={24}
                          rx={2}
                          fill={`${BASE_COLORS[b]}15`}
                          stroke={`${BASE_COLORS[b]}30`}
                          strokeWidth={1}
                        />
                        <text
                          x={109 + i * 22}
                    ...
                    .map((b, i) => (
                      <g key={`trans-mrna-${i}`}>
                        <rect
                          ...
                          height={24}
                          rx={2}
                          fill={`${BASE_COLORS[b]}20`}
                          stroke={`${BASE_COLORS[b]}40`}
                          strokeWidth={1}
                        />
                        <text
                          x={109 + i * 22}
                ...
                <div className="border-accent-cool/20 bg-accent-cool/5 rounded-lg border p-3">
                  <p className="text-fg-secondary text-xs leading-relaxed">
                    <span className="text-accent-cool font-medium">转录：</span>
                    RNA 聚合酶结合到基因的启动子区域，沿 DNA 模板链（3&apos;&rarr;5&apos;）移动，
                    合成互补的 mRNA 链（5&apos;&rarr;3&apos;）。转录后 mRNA
                    经过加工（加帽、加尾、剪接）后离开细胞核。
                  </p>
                </div>
              </motion.div>
            )}
            ...
            {mode === "translation" && (
              ...
              >
                  ...
                  <table className="w-full border-collapse font-mono text-[10px]">
                    <thead>
                      <tr className="border-border-faint border-b">
                        <th className="text-fg-muted px-2 py-1.5 text-left">密码子</th>
                        <th className="text-fg-muted px-2 py-1.5 text-left">氨基酸</th>
                        <th className="text-fg-muted px-2 py-1.5 text-left">状态</th>
                      </tr>
                    </thead>
                    <tbody>
                      {protein.map((p, i) => (
                        <motion.tr
                          ...
                          className={`border-border-faint/50 border-b ${
                        ...
                        >
                          ...
                          <td className="text-fg-primary px-2 py-1.5 font-bold">{p.aminoAcid}</td>
                          <td className="px-2 py-1.5">
                ...
                <div className="border-accent-cool/20 bg-accent-cool/5 rounded-lg border p-3">
                  <p className="text-fg-secondary text-xs leading-relaxed">
                    <span className="text-accent-cool font-medium">翻译：</span>
                    核糖体沿 mRNA 移动，每次读取 3 个碱基（一个密码子）。 tRNA
                    的反密码子与密码子配对，带来对应的氨基酸。 AUG
                    是起始密码子（甲硫氨酸），UAA/UAG/UGA 是终止密码子。
                  </p>
                </div>
              </motion.div>
            )}
          ...
          <div className="flex items-center gap-4">
            {Object.entries(BASE_COLORS).map(([base, color]) => (
              <div key={base} className="flex items-center gap-1.5">
                ...
                />
                <span className="text-fg-muted font-mono text-[9px]">{base}</span>
              </div>
            ))}
          </div>
