...
function createEraTag(era) {
  if (!era) return '';
  return `<span class="tag" style="color:${era.color};border-color:${era.color}30;background:${era.color}10">${escapeHtml(era.name)}</span>`;
}
...
function createFigureTags(figure) {
  ...
  return `
    ${createEraTag(era)}
    <span class="tag">${escapeHtml(REGION_LABELS[figure.region] || figure.region)}</span>
    <span class="tag">${escapeHtml(DOMAIN_LABELS[domain])}</span>
  `;
}
...
export function createFigureCard(figure, onSelect) {
  ...
  card.innerHTML = `
    <div class="figure-head">
      <div class="figure-avatar">${escapeHtml(figure.name[0])}</div>
      <div>
        <div class="figure-name">${escapeHtml(figure.name)}</div>
        <div class="figure-title-text">${escapeHtml(figure.title)}</div>
        <div class="figure-dates">${escapeHtml(getFigureDates(figure))}</div>
      </div>
    </div>
    <p class="figure-desc">${escapeHtml(figure.desc)}</p>
    ${figure.quote ? `<p class="figure-card-quote">"${escapeHtml(figure.quote)}"</p>` : ''}
    <div class="figure-meta-row">${createFigureTags(figure)}</div>
    ${figure.impact?.length ? `<div class="figure-impact-inline">${figure.impact.slice(0, 3).map(item => `<span>${escapeHtml(item)}</span>`).join('')}</div>` : ''}
...
function createRelatedFiguresSection(figure) {
  ...
  const items = relations.map(rel => {
    ...
    return `<div class="fm-rel-item" data-figure="${escapeHtml(other)}">
      <span class="fm-rel-badge" style="background:${color}20;color:${color};border:1px solid ${color}40">${escapeHtml(RELATION_LABELS[rel.type] || rel.type)}</span>
      <span class="fm-rel-name">${escapeHtml(other)}</span>
      <span class="fm-rel-desc">${escapeHtml(rel.desc || '')}</span>
    </div>`;
  }).join('');
...
function createEventLinksSection(figure) {
  ...
  const items = links.map(link => `<div class="fm-ev-item">${escapeHtml(link.eventTitle)}</div>`).join('');
    ...
    <div class="fm-section">
      ...
      <div class="fm-timeline">
        <div class="fm-tl-era" style="--era-color:${era.color}">
          <span class="fm-tl-era-name">${escapeHtml(era.name)}</span>
          <span class="fm-tl-era-range">${escapeHtml(eraStart < 0 ? '前' + Math.abs(eraStart) : String(eraStart))} — ${escapeHtml(eraEnd < 0 ? '前' + Math.abs(eraEnd) : String(eraEnd))}</span>
        </div>
        <div class="fm-tl-track">
          ...
          <div class="fm-tl-marker fm-tl-birth" style="left:${birthPct}%">
            ...
            <span class="fm-tl-label">${escapeHtml(figure.birth < 0 ? '前' + Math.abs(figure.birth) : String(figure.birth))} 出生</span>
          </div>
          ${figure.death != null ? `<div class="fm-tl-marker fm-tl-death" style="left:${deathPct}%">
            <span class="fm-tl-dot" style="background:${era.color}"></span>
            <span class="fm-tl-label">${escapeHtml(figure.death < 0 ? '前' + Math.abs(figure.death) : String(figure.death))} 逝世</span>
          </div>` : ''}
        </div>
      </div>
    </div>
...
function createAchievementsSection(figure) {
  ...
  return `<div class="fm-section"><div class="fm-section-title">主要成就</div><div class="fm-achievements">${items.map(item => `<div class="fm-ach-item"><span class="fm-ach-dot"></span>${escapeHtml(item)}</div>`).join('')}</div></div>`;
}

function createControversiesSection(figure) {
  const items = figure.controversies || [];
  if (items.length === 0) return '';
  return `<div class="fm-section"><div class="fm-section-title">争议与反思</div><div class="fm-controversies">${items.map(item => `<div class="fm-cont-item"><span class="fm-cont-icon">⚖</span>${escapeHtml(item)}</div>`).join('')}</div></div>`;
}

function createKeyEventsSection(figure) {
  const events = figure.keyEvents || [];
  if (events.length === 0) return '';
  return `<div class="fm-section"><div class="fm-section-title">生平大事年表</div><div class="fm-key-events">${events.map(ev => `<div class="fm-ke-item"><span class="fm-ke-year">${escapeHtml(ev.year < 0 ? '前' + Math.abs(ev.year) : String(ev.year))}</span><span class="fm-ke-dot"></span><span class="fm-ke-title">${escapeHtml(ev.title)}</span></div>`).join('')}</div></div>`;
}

function formatLongDesc(text) {
  if (!text) return '';
  return text.split('\n\n').map(p => `<p class="fm-paragraph">${escapeHtml(p)}</p>`).join('');
}
...
export function openFigureModal(figure) {
  ...
  overlay.innerHTML = `
    <div class="modal-card figure-modal-card figure-modal-enhanced" role="dialog" aria-modal="true" aria-label="${escapeHtml(figure.name)} · ${escapeHtml(figure.title)}">
      <div class="figure-modal-head">
        <div class="figure-avatar figure-modal-avatar">${escapeHtml(figure.name[0])}</div>
        <div class="figure-modal-id">
          <div class="figure-name">${escapeHtml(figure.name)}</div>
          <div class="figure-title-text">${escapeHtml(figure.title)}</div>
          <div class="figure-dates">${escapeHtml(getFigureDates(figure))} · ${escapeHtml(getFigureLifespan(figure))}</div>
        </div>
        <button class="modal-close" aria-label="关闭">✕</button>
      </div>
      <div class="figure-modal-tags">${createFigureTags(figure)}</div>
      ${createFigureTimeline(figure)}
      <div class="figure-modal-desc">${formatLongDesc(figure.longDesc) || escapeHtml(figure.desc)}</div>
      ${createAchievementsSection(figure)}
      ${impactItems.length ? `<div class="fm-section"><div class="fm-section-title">历史影响</div><div class="figure-impact-list">${impactItems.map(item => `<span>${escapeHtml(item)}</span>`).join('')}</div></div>` : ''}
