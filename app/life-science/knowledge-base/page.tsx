...
export default function LifeScienceKnowledgeBasePage() {
  const groups = lifeScienceKB.getArticlesByCategory();
      ...
      <div className="space-y-12">
        {groups.map((group) => (
          <section key={group.category}>
            <h2 className="text-accent-green mb-5 font-mono text-xs tracking-[0.28em] uppercase">
              {group.category}
            </h2>
            <div className="grid gap-4 sm:grid-cols-2">
              {group.articles.map((article) => (
                <Link
                  key={article.slug}
                  href={`/life-science/knowledge-base/${article.slug}`}
                ...
                >
                  <h3 className="text-fg-primary group-hover:text-accent-green mb-2 text-base font-semibold transition-colors">
                    {article.title}
                  </h3>
