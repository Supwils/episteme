...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description,
    openGraph: { title: `${species.name}（${species.nameEn}） — 生命科学`, description, images: [{ url: ogImage, width: 1200, height: 630 }] },
  };
}
...
export default async function SpeciesDetailPage({ params }: Props) {
  ...
  const related = allSpecies
    .filter((s) => s.id !== species.id && (s.era === species.era || s.taxonomy.phylum === species.taxonomy.phylum))
    .slice(0, 6);
      ...
      <div className="flex flex-col gap-12 lg:flex-row">
        <article className="min-w-0 flex-1 max-w-[1200px]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
            ...
            {species.taxonomy.phylum && <FactCard label="门" value={species.taxonomy.phylum} accent={accent} />}
            {species.taxonomy.class && <FactCard label="纲" value={species.taxonomy.class} accent={accent} />}
            {species.taxonomy.order && <FactCard label="目" value={species.taxonomy.order} accent={accent} />}
          </div>

          <FadeInSection className="mb-12">
            <h2 className="font-display text-fg-primary text-xl font-semibold mb-4" id="key-traits">关键特征</h2>
            <div className="flex flex-wrap gap-2">
              {species.keyTraits.map((trait) => (
                <span
                  key={trait}
                  className="rounded-full border px-3 py-1.5 font-mono text-[11px] tracking-[0.08em]"
                  style={{ borderColor: `${accent}25`, color: `${accent}cc`, backgroundColor: `${accent}08` }}
                >
                  {trait}
                </span>
              ))}
          ...
          <FadeInSection className="mb-12">
            <h2 className="font-display text-fg-primary text-xl font-semibold mb-4" id="taxonomy">分类信息</h2>
            <div className="border-border-faint bg-bg-near border p-6">
              <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <TaxonomyEntry label="界" value={species.taxonomy.kingdom} />
                {species.taxonomy.phylum && <TaxonomyEntry label="门" value={species.taxonomy.phylum} />}
          ...
          {["动物界", "植物界", "真菌界"].includes(species.taxonomy.kingdom) && (
            <FadeInSection className="mb-12">
              <h2 className="font-display text-fg-primary text-xl font-semibold mb-4" id="cell-structure">细胞结构</h2>
              <div className="border-border-faint bg-bg-near border p-6">
                <CellExplorer />
              </div>
            </FadeInSection>
        ...
        <aside className="w-full lg:w-80 flex-shrink-0">
          <div className="sticky top-24 space-y-6">
            ...
            <div className="border-border-faint bg-bg-near border p-5">
              <h3 className="font-display text-fg-primary text-sm font-semibold mb-4 tracking-wide">相关物种</h3>
                  ...
                  <li key={r.id}>
                    ...
                    >
                      ...
                      <span>
                        ...
                        <span className="text-fg-muted ml-1.5 font-mono text-[10px] italic">{r.nameEn}</span>
                      </span>
                    </Link>
                  </li>
                ))}
