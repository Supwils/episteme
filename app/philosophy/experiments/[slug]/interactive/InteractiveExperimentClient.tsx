...
const TrolleyProblem = dynamic(
  () => import("@/subjects/philosophy/components/experiments/TrolleyProblem")
);
const PlatosCave = dynamic(() => import("@/subjects/philosophy/components/experiments/PlatosCave"));
const ChineseRoom = dynamic(
  () => import("@/subjects/philosophy/components/experiments/ChineseRoom")
);
const ExperienceMachine = dynamic(
  () => import("@/subjects/philosophy/components/experiments/ExperienceMachine")
);
const VeilOfIgnorance = dynamic(
  () => import("@/subjects/philosophy/components/experiments/VeilOfIgnorance")
);
const ShipOfTheseus = dynamic(
  () => import("@/subjects/philosophy/components/experiments/ShipOfTheseus")
);
const BuridansAss = dynamic(
  () => import("@/subjects/philosophy/components/experiments/BuridansAss")
);

const EXPERIMENT_MAP: Record<
  string,
  { component: React.ComponentType; title: string; titleEn: string }
> = {
  ...
  "experience-machine": {
    component: ExperienceMachine,
    title: "体验机器",
    titleEn: "Experience Machine",
  },
  "veil-of-ignorance": {
    component: VeilOfIgnorance,
    title: "无知之幕",
...
export default function InteractiveExperimentClient() {
  ...
  if (!entry) {
    return (
      <div className="mx-auto w-full max-w-[1800px] px-6 py-12 text-center sm:px-10 sm:py-16 lg:px-16">
  ...
  return (
    <div className="mx-auto w-full max-w-[1800px] px-6 py-12 sm:px-10 sm:py-16 lg:px-16">
      <Link
      ...
      <header className="mb-8">
        ...
        <p className="text-fg-muted font-display text-lg tracking-wide italic">{entry.titleEn}</p>
      </header>
